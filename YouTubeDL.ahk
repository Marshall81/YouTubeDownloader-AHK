...
Gui, Add, Button,hwndAbout x232 yP w65 h23 -wrap gAbout , Über
		...
		IfMsgBox, Yes	
		{
			...
			(
				...
				UpdateBlock:
				{
					...
					MsgBox, 36,, Neustarten um das Update abzuschließen?
					IfMsgBox, Yes	
					{
...
return
	...
	Progress:																								;--------- FUNCTION FOR CALCULATING PROGRESS VARIABLE---------
	{
		...
		Progr:
		{
			if (done=1)
			{
				SB_SetText("Download beendet.",2)
				SB_SetIcon("%systemroot%\system32\shell32.dll" , 297, 1)
				SB_SetText("Gedownloadet",2)
				...
				sleep 130
				MsgBox, 64,, Download erfolgreich!
				reload
			}
			else
			{
...
Start:
{
	...
	If OutputVar= Start																			;--------- START BUTTON TRIGGER---------
	{
		...
		SB_SetText(" Empfange die Metadaten.",2)
		...
		SB_SetText("Video Datei importiert, starte jetzt den Download.",2)
		SB_SetIcon("%systemroot%\system32\wmploc.dll" , 126, 1)
	...
	If OutputVar= Pause																		;--------- PAUSE BUTTON TRIGGER---------
	{
		pause , off
		WinKill,ahk_pid %Pid%
		GuiControl,,Down,Weiter
		SetTaskbarProgress(perc,"P")
		AddTooltip(Start,"Download fortsetzen")
		SB_SetText("Paused",2)
	...
	If OutputVar= Weiter																		;--------- RESUME BUTTON TRIGGER---------
	{
